...
def append_star_unlock_items(items):
    ...
    # Do not append any placeholder slots — return items unchanged
    return items
...
def is_star_unlock(label: str) -> bool:
    # Disable star/unlock detection so these items are treated as normal
    return False


def is_premium(label: str) -> bool:
    # Disable premium detection globally — treat all tools as non-premium
    return False
